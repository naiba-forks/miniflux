...
func RefreshFeed(store *storage.Storage, userID, feedID int64, forceRefresh bool) *locale.LocalizedErrorWrapper {
	...
	if originalFeed.FeedSourceType == "web_scraper" {
		...
		// When JS rendering is enabled, use Obscura headless browser to render
		...
		if originalFeed.UseJSRender && config.Opts.ObscuraEnabled() {
			slog.Debug("Rendering web scraper listing page with Obscura",
				slog.Int64("user_id", userID),
				slog.Int64("feed_id", feedID),
