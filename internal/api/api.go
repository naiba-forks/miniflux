...
func NewHandler(store *storage.Storage, pool *worker.Pool) http.Handler {
	...
	mux.HandleFunc("POST /v1/entries/{entryID}/summarize", handler.summarizeEntry)
	mux.HandleFunc("POST /v1/ai/backfill", handler.backfillAISummaries)
	mux.HandleFunc("POST /v1/ai/force-backfill", handler.forceBackfillAISummaries)
	mux.HandleFunc("POST /v1/ai/page-summary", handler.generateAIPageSummary)
	mux.HandleFunc("GET /v1/ai/backfill-status", handler.getBackfillStatus)
	mux.HandleFunc("POST /v1/ai/stop-backfill", handler.stopBackfill)
