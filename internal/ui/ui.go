...
func Serve(store *storage.Storage, pool *worker.Pool) http.Handler {
	...
	// AI Digest page.
	mux.HandleFunc("GET /ai-digest", handler.showAIDigestPage)
	...
	mux.HandleFunc("POST /ai-backfill", handler.aiBackfill)
	mux.HandleFunc("POST /ai-force-backfill", handler.aiForceBackfill)
	mux.HandleFunc("GET /ai-backfill-status", handler.aiBackfillStatus)
	mux.HandleFunc("POST /ai-stop-backfill", handler.aiStopBackfill)
	mux.HandleFunc("POST /ai-test-connection", handler.aiTestConnection)
	mux.HandleFunc("POST /ai-page-summary", handler.aiPageSummary)
	mux.HandleFunc("GET /ai-page-summary-status", handler.aiPageSummaryStatus)
